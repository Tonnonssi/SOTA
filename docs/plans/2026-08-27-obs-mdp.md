...
- 실행 환경: Isaac Lab 이 설치된 파이썬 환경(이 저장소에서는 conda `env_isaaclab`)을 활성화하고 저장소 루트에서 `pytest isaac`. MuJoCo 비교 테스트만 mujoco 가 있는 환경(`lerobot`)에서 돌린다. ROS 가 sourced 된 셸이라면 `PYTHONPATH`를 비워야 한다 — ROS 의 `launch_testing` 이 pytest 플러그인으로 로드돼 수집 단계에서 죽는다.
...
- Produces (이슈 C가 그대로 쓴다):
  ...
  - `push(rot_his, act_his, quat_xyzw (N,4), action (N,6), skip_mask: (N,) bool | None = None) -> obs (N,40)` — 제자리로 최신을 index 0에 밀어 넣고 `flatten` 결과를 돌려준다. `skip_mask`가 True인 env는 밀어 넣지 않는다 — 방금 리셋된 env의 첫 관측은 실기처럼 리셋값 그대로여야 하므로 env는 `skip_mask=reset_buf`를 넘긴다 (스펙 §9.3 정정). 이력에 넣는 `action`은 **클립 전** 정책 출력이다.
...
```bash
pytest isaac/tests/test_obs_layout.py -v
```
...
def push(rot_his: torch.Tensor, act_his: torch.Tensor,
    ...
    첫 관측은 실기처럼 리셋값 그대로여야 한다 (모듈 docstring "리셋 직후" 참조).
    """
...
```bash
pytest isaac/tests/test_mdp.py -v
```
...
```bash
pytest isaac/tests/test_mdp.py -v -k "terminated or truncat or corner or quat_dist"
```
