...
> **2026-08-28 결과 (이슈 #15 첫 전량 학습).** 축 1: 완주 64/64, 전진 0.0001 m/s, 높이 0.048 m —
> 3개 중 1개 통과. 정책은 6관절 중 5개를 ±50° 한계에 박고 굳었다. 축 2: `walk.onnx` 는 우리
> 시뮬에서 관절 순서 3종 × 쿼터니언 부호 2종 전부 넘어진다 — 위 문단이 예고한 "섹션 2가
> 덜 끝났다" 쪽이다. 원인 분석(Table IV 의 예산이 정지를 선호한다 + 관절 한계 클립이 탐색을
> 지운다)과 후속 손잡이는 `docs/reports/2026-08-28-walk-freeze-analysis.md`.
...
5. **Table IV 그대로는 굳는다 (2026-08-28).** 정지 보너스 28/스텝 vs 논문 속도의 progress 0.75/스텝,
   action cost 는 크기가 아니라 변화량이라 mu 가 한계 밖으로 도망가며 탐색을 지운다. 논문이 같은
   표로 걸은 이유(시뮬 dt / 2단계 노이즈 / 131k env)는 미검증. `docs/reports/2026-08-28-walk-freeze-analysis.md` §6, §8.
